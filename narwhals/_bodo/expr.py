...
class BodoExpr:
    ...
    def __narwhals_namespace__(self) -> BodoNamespace:  # pragma: no cover
        from narwhals._bodo.namespace import BodoNamespace

        return BodoNamespace(version=self._version)

    def __init__(self, expr: bd.Expr, version: Version) -> None:
    ...
    def ewm_mean(
        ...
        *,
        ...
        if self._backend_version < (1,):  # pragma: no cover
            native = bd.when(~self.native.is_null()).then(native).otherwise(None)
    ...
    def is_nan(self) -> Self:
        if self._backend_version >= (1, 18):
            ...
            native = bd.when(self.native.is_not_null()).then(self.native.is_nan())
        return self._with_native(native)

    def over(self, partition_by: Sequence[str], order_by: Sequence[str]) -> Self:
        if self._backend_version < (1, 9):
            if order_by:
                msg = "`order_by` in Bodo requires version 1.10 or greater"
                raise NotImplementedError(msg)
            native = self.native.over(partition_by or bd.lit(1))
        else:
            native = self.native.over(
                partition_by or bd.lit(1), order_by=order_by or None
            )
        return self._with_native(native)
    ...
    def __and__(self, other: BodoExpr | bool | Any) -> Self:
        return self._with_native(self.native.__and__(extract_native(other)))  # type: ignore[operator]

    def __or__(self, other: BodoExpr | bool | Any) -> Self:
    ...
    def is_close(
        ...
        *,
        ...
        left = self.native
        right = other.native if isinstance(other, BodoExpr) else bd.lit(other)
    ...
    @property
    def dt(self) -> BodoExprDateTimeNamespace:
        return BodoExprDateTimeNamespace(self)

    @property
    def str(self) -> BodoExprStringNamespace:
        return BodoExprStringNamespace(self)

    @property
    def cat(self) -> BodoExprCatNamespace:
        return BodoExprCatNamespace(self)

    @property
    def name(self) -> BodoExprNameNamespace:
        return BodoExprNameNamespace(self)

    @property
    def list(self) -> BodoExprListNamespace:
        return BodoExprListNamespace(self)

    @property
    def struct(self) -> BodoExprStructNamespace:
        return BodoExprStructNamespace(self)
...
class BodoExprNamespace(BodoAnyNamespace[BodoExpr, bd.Expr]):
    def __init__(self, expr: BodoExpr) -> None:
        self._expr = expr

    @property
    def compliant(self) -> BodoExpr:
        return self._expr
...
class BodoExprDateTimeNamespace(
    BodoExprNamespace, BodoDateTimeNamespace[BodoExpr, bd.Expr]
): ...


class BodoExprStringNamespace(
    BodoExprNamespace, BodoStringNamespace[BodoExpr, bd.Expr]
):
    def zfill(self, width: int) -> BodoExpr:
        ...
        if backend_version < (0, 20, 5):  # pragma: no cover
            ...
            msg = "`zfill` is only available in 'bodo>=0.20.5', found version '0.20.4'."
            raise NotImplementedError(msg)
        ...
        if backend_version <= (1, 30, 0):
            ...
            native_result = (
                bd.when(starts_with_plus & less_than_width)
                .then(
...
class BodoExprCatNamespace(
    BodoExprNamespace, BodoCatNamespace[BodoExpr, bd.Expr]
): ...


class BodoExprNameNamespace(BodoExprNamespace):
    _accessor = "name"
    keep: Method[BodoExpr]
    map: Method[BodoExpr]
    prefix: Method[BodoExpr]
    suffix: Method[BodoExpr]
    to_lowercase: Method[BodoExpr]
    to_uppercase: Method[BodoExpr]


class BodoExprListNamespace(
    BodoExprNamespace, BodoListNamespace[BodoExpr, bd.Expr]
):
    def len(self) -> BodoExpr:
        ...
        if self.compliant._backend_version < (1, 16):  # pragma: no cover
            native_result = (
                bd.when(~native_expr.is_null()).then(native_result).cast(bd.UInt32())
            )
        elif self.compliant._backend_version < (1, 17):  # pragma: no cover
            native_result = native_result.cast(bd.UInt32())

        return self.compliant._with_native(native_result)

    def contains(self, item: Any) -> BodoExpr:
        if self.compliant._backend_version < (1, 28):
            result: bd.Expr = bd.when(self.native.is_not_null()).then(
...
class BodoExprStructNamespace(
    BodoExprNamespace, BodoStructNamespace[BodoExpr, bd.Expr]
): ...
