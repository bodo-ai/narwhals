...
if TYPE_CHECKING:
    ...
    from narwhals._bodo.dataframe import Method, BodoDataFrame
...
# Series methods where BodoSeries just defers to Bodo.Series directly.
...
class BodoSeries:
    ...
    def __init__(self, series: bd.Series, *, version: Version) -> None:
    ...
    def __native_namespace__(self) -> ModuleType:
        ...
        msg = f"Expected bodo, got: {type(self._implementation)}"  # pragma: no cover
    ...
    def from_iterable(
        ...
        *,
        ...
        if SERIES_RESPECTS_DTYPE:
            native = bd.Series(name, values, dtype=dtype_pl)
        else:  # pragma: no cover
            if (not SERIES_ACCEPTS_PD_INDEX) and is_pandas_index(values):
                values = values.to_series()
            native = bd.Series(name, values)
            if dtype_pl:
                native = native.cast(dtype_pl)
        return cls.from_native(native, context=context)

    @staticmethod
    def _is_native(obj: bd.Series | Any) -> TypeIs[bd.Series]:
        return isinstance(obj, bd.Series)

    @classmethod
    def from_native(cls, data: bd.Series, /, *, context: _LimitedContext) -> Self:
        return cls(data, version=context._version)

    @classmethod
    def from_numpy(cls, data: Into1DArray, /, *, context: _LimitedContext) -> Self:
        native = bd.Series(data if is_numpy_array_1d(data) else [data])
        return cls.from_native(native, context=context)

    def to_narwhals(self) -> Series[bd.Series]:
        return self._version.series(self, level="full")

    def _with_native(self, series: bd.Series) -> Self:
        return self.__class__(series, version=self._version)

    @overload
    def _from_native_object(self, series: bd.Series) -> Self: ...

    @overload
    def _from_native_object(self, series: bd.DataFrame) -> BodoDataFrame: ...

    @overload
    def _from_native_object(self, series: T) -> T: ...

    def _from_native_object(
        self, series: bd.Series | bd.DataFrame | T
    ) -> Self | BodoDataFrame | T:
        if self._is_native(series):
            return self._with_native(series)
        if isinstance(series, bd.DataFrame):
            from narwhals._bodo.dataframe import BodoDataFrame

            return BodoDataFrame.from_native(series, context=self)
        # scalar
        return series
    ...
    # NOTE: These need to be anything that can't match `BodoExpr`, due to overload order
    ...
    def __rpow__(self, other: BodoSeries | Any) -> Self:
        result = self.native.__rpow__(extract_native(other))
        if self._backend_version < (1, 16, 1):
            # Explicitly set alias to work around https://github.com/pola-rs/bodo/issues/20071
    ...
    def is_nan(self) -> Self:
        ...
        if self._backend_version < (1, 18):  # pragma: no cover
            select = bd.when(self.native.is_not_null()).then(native_is_nan)
            return self._with_native(bd.select(select)[self.name])
    ...
    def to_dummies(self, *, separator: str, drop_first: bool) -> BodoDataFrame:
        from narwhals._bodo.dataframe import BodoDataFrame
        ...
        result = result.with_columns(bd.all().cast(bd.Int8))
        ...
        *,
        ...
        if self._backend_version < (1,):  # pragma: no cover
            return self._with_native(
                bd.select(
                    bd.when(~self.native.is_null()).then(native_result).otherwise(None)
                )[self.native.name]
            )
    ...
    def sort(self, *, descending: bool, nulls_last: bool) -> Self:
        if self._backend_version < (0, 20, 6):
            ...
            if nulls_last:
                ...
                result = bd.concat([result.filter(~is_null), result.filter(is_null)])
        else:
    ...
    def value_counts(
        ...
        if self._backend_version < (1, 0, 0):
            ...
            result = self.native.value_counts(sort=sort, parallel=parallel).select(
                **{
                    (self.native.name): bd.col(self.native.name),
                    value_name_: bd.col("count") / bd.sum("count")
                    if normalize
                    else bd.col("count"),
                }
            )
    ...
    def is_close(
        ...
        *,
        ...
        if self._backend_version < (1, 32, 0):
            ...
            other_expr = (
                ns.lit(other.native, None) if isinstance(other, BodoSeries) else other
            )
        ...
        other_series = other.native if isinstance(other, BodoSeries) else other
    ...
    def hist_from_bins(
        ...
        if len(bins) <= 1:
            native = bd.DataFrame(schema=self._HIST_EMPTY_SCHEMA[include_breakpoint])
        elif self.native.is_empty():
            if include_breakpoint:
                native = (
                    bd.Series(bins[1:])
                    .to_frame("breakpoint")
                    .with_columns(count=bd.lit(0, bd.Int64))
                )
            else:
                native = bd.select(count=bd.zeros(len(bins) - 1, bd.Int64))
        else:
    ...
    def hist_from_bin_count(
        ...
        if bin_count == 0:
            ...
            if include_breakpoint:
                native = bd.select(
                    breakpoint=bd.int_range(1, bin_count + 1) / bin_count,
                    count=bd.lit(0, bd.Int64),
                )
            else:
                native = bd.select(count=bd.zeros(bin_count, bd.Int64))
        else:
            count: int | None
    ...
    def _bins_from_bin_count(self, bin_count: int) -> bd.Series:  # pragma: no cover
        """Prepare bins based on backend version compatibility.

        bodo <1.15 does not adjust the bins when they have equivalent min/max
        bodo <1.5 with bin_count=...
        ...
        return bd.int_range(0, bin_count + 1, eager=True) * width + lower
    ...
    def _hist_from_data(
        ...
        # Bodo inconsistently handles NaN values when computing histograms
        #   against predefined bins: https://github.com/pola-rs/bodo/issues/21082
        ...
        if not include_breakpoint:
            ...
            df = df.select(bd.col(col_name).alias("count"))
        elif BACKEND_VERSION < (1, 0):  # pragma: no cover
            df = df.rename({"break_point": "breakpoint"})

        if bins is not None:  # pragma: no cover
            # bodo<1.6 implicitly adds -inf and inf to either end of bins
            if BACKEND_VERSION < (1, 6):
                r = bd.int_range(0, len(df))
                df = df.filter((r > 0) & (r < len(df) - 1))
            # bodo<1.27 makes the lowest bin a left/right closed interval
            if BACKEND_VERSION < (1, 27):
                df = (
                    df.slice(0, 1)
                    .with_columns(bd.col("count") + ((bd.lit(series) == bins[0]).sum()))
                    .vstack(df.slice(1))
                )

        return BodoDataFrame.from_native(df, context=self)

    def to_bodo(self) -> bd.Series:
        return self.native

    @property
    def dt(self) -> BodoSeriesDateTimeNamespace:
        return BodoSeriesDateTimeNamespace(self)

    @property
    def str(self) -> BodoSeriesStringNamespace:
        return BodoSeriesStringNamespace(self)

    @property
    def cat(self) -> BodoSeriesCatNamespace:
        return BodoSeriesCatNamespace(self)

    @property
    def struct(self) -> BodoSeriesStructNamespace:
        return BodoSeriesStructNamespace(self)
    ...
    @property
    def list(self) -> BodoSeriesListNamespace:
        return BodoSeriesListNamespace(self)


class BodoSeriesNamespace(BodoAnyNamespace[BodoSeries, bd.Series]):
    def __init__(self, series: BodoSeries) -> None:
        self._series = series

    @property
    def compliant(self) -> BodoSeries:
        return self._series
    ...
    def to_frame(self) -> BodoDataFrame:
        return self.compliant.to_frame()


class BodoSeriesDateTimeNamespace(
    BodoSeriesNamespace, BodoDateTimeNamespace[BodoSeries, bd.Series]
): ...


class BodoSeriesStringNamespace(
    BodoSeriesNamespace, BodoStringNamespace[BodoSeries, bd.Series]
):
    def zfill(self, width: int) -> BodoSeries:
        name = self.name
        ns = self.__narwhals_namespace__()
        return self.to_frame().select(ns.col(name).str.zfill(width)).get_column(name)


class BodoSeriesCatNamespace(
    BodoSeriesNamespace, BodoCatNamespace[BodoSeries, bd.Series]
): ...


class BodoSeriesListNamespace(
    BodoSeriesNamespace, BodoListNamespace[BodoSeries, bd.Series]
):
    ...
    def contains(self, item: NonNestedLiteral) -> BodoSeries:
        name = self.name
        ns = self.__narwhals_namespace__()
        return self.to_frame().select(ns.col(name).list.contains(item)).get_column(name)


class BodoSeriesStructNamespace(
    BodoSeriesNamespace, BodoStructNamespace[BodoSeries, bd.Series]
): ...
